...
**Huawei Cloud skill discovery:** for any Huawei Cloud task, search installed skills and `~/.agents/INSTALLABLE_SKILLS.json` **together** as one candidate pool — the catalog is not a fallback behind the installed list. Rank by how well the skill's name and declared trigger keywords match the task's **action verb + object** (e.g. a skill whose trigger says "create alarm" for a *create alarm* task), not by how loosely its description mentions the topic noun. An installed skill that only shares the noun (e.g. a query-only skill mentioning "ECS") does **not** outrank a catalog skill whose trigger matches the action — installation status is not a ranking factor. Recalled memory is context about past sessions, not a skill-selection shortcut — never let it bypass or shortcut the discovery procedure above, even if a recalled note seems to point at a specific tool or approach.

1. **Installed match** → invoke via `load_skill`.
2. **Catalog-only match (unique)** → run its `install_cmd` via `shell`, then invoke via `load_skill` in the same turn (the newly installed skill is immediately loadable — no restart needed).
3. **Multiple matches** → list them and ask the user which to install.
4. **No match** → if `huawei-cloud-find-skills` is installed, invoke it; else install it from the catalog and invoke it.

The catalog is a dict with a `skills` array; iterate `data["skills"]`, not `data`. Each entry has `install_cmd` with `cmd` and `args`; joining them yields the install command (e.g. `npx skills add https://gitcode.com/huaweicloud/huaweicloud-skills.git --skill huawei-cloud-billing-scout -g -y`). The searchable text is in `entry["frontmatter"]["description"]` (not a top-level `description` — that field is absent); the description contains a `Trigger:` line of example phrases — match the task's action verb + object against that Trigger line.

After the skill chain is exhausted, fall back to: local API snapshot grep → websearch/webfetch.
